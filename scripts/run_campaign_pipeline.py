...
from project_paths import BRAND_DNA_DIR, RESULTS_DIR, SOURCE_DIR, PROJECT_ROOT
...
from sns_labeling.product_grounding import (
...
def _run_imc_pipeline(args) -> int:
    ...
    if "higgsfield" in args.providers:
        ...
        for p in args.providers:
            if p == "higgsfield":
                expanded.extend(["hf_gpt", "hf_mkt"])
            else:
                expanded.append(p)
        args.providers = list(dict.fromkeys(expanded))
    ...
    # ---- Step 0: single-product outfit selection ----
    outfit_manifest = None
    if args.product_grounded:
        print("\n[Step 0] Single-product outfit selection")
        outfit_manifest = build_outfit_manifest(spec, focus=args.product_focus)
        if outfit_manifest is None:
            print("  [warn] no top/bottom product matched; falling back to text-only wardrobe brief")
        else:
            products_dir.mkdir(parents=True, exist_ok=True)
            write_outfit_manifest(outfit_manifest, products_dir)
            selected = outfit_manifest.get("selected_match") or {}
            selected_role = outfit_manifest.get("selected_role")
            companion = "bottom" if selected_role == "top" else "top"
            print(f"  selected {selected_role}: {selected.get('brand_folder')}/{selected.get('season')} "
                  f"{selected.get('code_folder')}/{selected.get('lifestyle_folder')}/"
                  f"{Path(selected.get('hero_image', '')).name}")
            print(f"  generated by model: {companion}, shoes, bag, accessories, props")
    else:
        print("\n[Step 0] Single-product outfit selection skipped (--no-product-grounded)")
    ...
    refs_by_scene = select_imc_refs(
        ...
        pool_path=args.pool_path or DEFAULT_POOL,
    ...
    if args.no_moments or not _has_gemini_key():
        reason = "--no-moments" if args.no_moments else "GEMINI_API_KEY not set"
        print(f"  [skip] {reason}; prompt builder will use fallback moments")
        moments_by_key = {}
    else:
        try:
    ...
    if args.no_images:
        ...
        if any(p in HF_PROVIDER_MODELS for p in args.providers):
...
def main() -> int:
    ...
    ap.add_argument("--pool-path", default=None,
                    help="Reference pool JSONL override. Defaults to "
    ...
    ap.add_argument("--no-moments", action="store_true",
                    help="skip Gemini moment extraction and use fallback moment text")
    ap.add_argument("--product-grounded", dest="product_grounded",
                    action="store_true", default=True,
                    help="Resolve exactly one top-or-bottom product before IMC image "
                         "generation and attach only that product image to the image model "
                         "(default true in IMC mode).")
    ap.add_argument("--no-product-grounded", dest="product_grounded",
                    action="store_false",
                    help="Use the legacy text-only wardrobe prompt without product images.")
    ap.add_argument("--product-focus", choices=["auto", "top", "bottom"], default="auto",
                    help="Which single garment to lock from product resources. "
                         "auto follows the IMC hero category when possible; default auto.")
    ap.add_argument("--providers", nargs="+",
                    default=["gpt"],
                    choices=["gpt", "gemini", "higgsfield", "hf_gpt", "hf_mkt"],
                    help="Direct image-gen providers (default: gpt only). "
                         ...
                         "Use hf_gpt/hf_mkt to write a Higgsfield MCP execution "
                         "plan for gpt_image_2/marketing_studio_image. `gemini` "
                         "is retained as opt-in for compat only. See SKILL.md")
    ...
    ap.add_argument("--apply-tryon", dest="apply_tryon", action="store_true",
                    ...
                    help="Run Step C+ virtual try-on after image generation "
                         "(default false; product-grounded generation is now the default)")
    ...
    refs = selector(
        ...
        k=args.k,
        pool_path=args.pool_path or (
            DEFAULT_POOL_PRODUCT if args.mood_mode == "product" else DEFAULT_POOL
        ),
        brand_filter=args.brand_filter,
        confidence_floor=args.confidence_floor,
    )
